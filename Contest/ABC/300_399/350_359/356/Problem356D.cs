...
public class ProblemD
{
    ...
    public void Solve()
    {
        ...
        if (binaryList.Any())
        {
            mask = (1L << binaryList.Count) - 1;
        }

        var maxNum = M & mask;
        var oneCount = BitOperations.PopCount((ulong)maxNum);
        var zeroCount = binaryList.Count - oneCount;
        ...
        {
            var oneComb = MathLibrary.Combination(oneCount, i);
            var comb = 1L << zeroCount;
            count += oneComb * comb;
        }
